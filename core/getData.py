...
class Data():
    ...
    def get_query_history_k_data_plus(self):
        ...
        if self.query_history_k_data_plus is not None:
    ...
    def get_query_all_stock(self, day = None):
        self.query_all_stock = self.cf.query_all_stock(day=day)

    def get_query_stock_basic(self):
        self.query_stock_basic = self.cf.query_stock_basic()
    ...
    def update_stock_industry(self):
        self.query_all_stock = self.cf.query_all_stock()
        self.query_all_stock.to_csv(os.path.join(PROJECT_PATH, 'data_info', '证券代码.csv'), index=0)

        self.query_stock_basic = self.cf.query_stock_basic()
        self.query_stock_basic.to_csv(os.path.join(PROJECT_PATH, 'data_info', '证券资料.csv'), index=0)
